    ...
    if dialect_name == 'sqlite':
        ...
        (
            EXISTS (
                ...
                AND LOWER(CAST(message.content AS TEXT)) LIKE '%' || :{key} || '%'
            )
            OR EXISTS (
                SELECT 1
...
class ChatTable:
    ...
    async def get_chat_by_id_for_user(
        ...
        *,
        ...
        chat = await self.get_chat_by_id_and_user_id(
            id,
            user.id,
            db=db,
        ...
        chat = await self.get_chat_by_id(id, db=db, include_messages=include_messages)
        if not chat:
            return None
